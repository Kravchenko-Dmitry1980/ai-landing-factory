...
<#
.SYNOPSIS
  Run live project verdict gate on the newest project (no manual UUID).
...
Write-Host "Live project verdict (latest) — backend: $backendUrl"

& $PythonExe $ScriptPath --latest --backend-url $backendUrl --runtime-root $RootDir
exit $LASTEXITCODE
